...
namespace AstralShift.QTI.NodeEditor
{
    ...
    public static class NodeEditorUtilities
    {
        ...
        private static Texture2D _scriptIcon = (EditorGUIUtility.IconContent("cs Script Icon").image as Texture2D);

        /// Saves Attribute from Type+Field for faster lookup. Resets on recompiles.
        private static Dictionary<Type, Dictionary<string, Dictionary<Type, Attribute>>> _typeAttributes =
        ...
        private static Dictionary<Type, Dictionary<string, List<PropertyAttribute>>> _typeOrderedPropertyAttributes =
        ...
        public static bool GetCachedAttrib<T>(Type classType, string fieldName, out T attribOut) where T : Attribute
        {
            ...
            if (!_typeAttributes.TryGetValue(classType, out typeFields))
            {
                typeFields = new Dictionary<string, Dictionary<Type, Attribute>>();
                _typeAttributes.Add(classType, typeFields);
            }
        ...
        public static List<PropertyAttribute> GetCachedPropertyAttribs(Type classType, string fieldName)
        {
            ...
            if (!_typeOrderedPropertyAttributes.TryGetValue(classType, out typeFields))
            {
                typeFields = new Dictionary<string, List<PropertyAttribute>>();
                _typeOrderedPropertyAttributes.Add(classType, typeFields);
            }
